...
from app.schema.chat_room_schema import ChatMessage, MessageType , UserInfo ,RoomInfo
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, username: str, room_name: str = "general"):
        """
        ...
        Args:
            ...
            room_name: The room ID to join (default: "general")
        """
        ...
        if username in self.users:
            ...
            self.users[username].current_room  =room_name        
        ...
        # add user to room
        await self._add_user_to_room(username,room_name)
        ...
        await self._broadcast_to_room_safe(room_name,join_message.dict())
        logger.info(f"User {username} connected to room {room_name}")
    ...
    async def disconnect(self, username: str):
        ...
        if username in self.users:
            ...
            if room_name :
                 await  self._remove_user_from_room(username,room_name)
            ...
            await self._broadcast_to_room_safe(room_name,leave_message.dict())  
        logger.info(f"User {username} disconnected from room {room_name}")      
        
    async def _add_user_to_room(self,username : str, room_name: str):
    ...
    async def _remove_user_from_room(self,username : str, room_name: str):
        ...
        if room_name in self.rooms:
            ...
            if not room.users:
                ...
                logger.info(f"Room {room_name} deleted (no users left)")    
    ...
    async def broadcast_to_room(self,room_name:str,message:dict):
        ...
        if room_name in self.rooms and message.get("message_type") == MessageType.CHAT:
            self.rooms[room_name].message_count +=1
            logger.info(self.rooms[room_name].message_count)
            
        await   self._broadcast_to_room_safe(room_name,message)
        
            
        
    async def _broadcast_to_room_safe(self, room_name: str, message:dict,exclude_user: str = None):
    ...
    async def switch_user_room(self, username: str, new_room_name: str) -> bool:
        """
        Switch a user from their current room to a new room.
        
        Args:
            username: The username to switch
            new_room_name: The new room to join
        ...
        # Add to new room
        user.current_room = new_room_name
        await self._add_user_to_room(username, new_room_name)
        ...
        await self._broadcast_to_room_safe(new_room_name, join_message.dict(), exclude_user=username)
        
        logger.info(f"User {username} switched from {old_room} to {new_room_name}")
        return True
    ...
    def get_user_count_by_room(self) -> Dict[str, int]:
        ...
        return {room_name: len(room.users) for room_name, room in self.rooms.items()}
